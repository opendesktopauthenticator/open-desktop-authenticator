...
#
#   config-*  TLS keys, nginx, log rotation, ufw, fail2ban, sshd hardening,
#             /usr/local/sbin.
...
tar -czf "$config_stage" \
  -C "$config_root" etc/letsencrypt etc/nginx etc/logrotate.d/nginx etc/ufw etc/fail2ban \
       etc/ssh/sshd_config.d etc/sysctl.d/99-hardening.conf usr/local/sbin 2>/dev/null
